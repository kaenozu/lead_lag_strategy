...
try {
  ...
  // dotenv load error is expected during testing
}
...
/**
 * データ不足時のマージン（営業日）
 * バックテストのウォームアップ期間に追加して、データ不足判定の余裕を持たせる
 */
const DATA_MARGIN_DAYS = 10;

/**
 * シグナル生成のデータ取得窓（カレンダー日）
 * windowLength + 160 の最低保障（280日）を設定
 * 理由: 日米アライメントによる行欠損・祝日・休場を考慮
 */
const SIGNAL_MIN_WINDOW_DAYS = 280;
