...
/** データ取得（日本・米国）は起動時に自動決定し runtime-data-source.json に記録。既定は日米とも Yahoo（OVERRIDE_* で公式 API 等に切替） */
const DEFAULT_DATA_MODE = 'yahoo';
const DEFAULT_US_OHLCV_PROVIDER = 'yahoo';
...
const BACKTEST_DEFAULTS = {
  WINDOW_LENGTH: 60,
  N_FACTORS: 3,
  LAMBDA_REG: 0.80,  // 最適化版：0.9→0.99→0.80（直近 1 年分析結果に基づく）
  ...
  WALKFORWARD_TRAIN_DAYS: 504,
  WALKFORWARD_TEST_DAYS: 63,
  WALKFORWARD_STEP_DAYS: 21
};
...
const CONSECUTIVE_LOSS_RULE = {
  THRESHOLD: 2,      // 2 日連続損失（分析結果に基づく最適値）
...
const config = {
  ...
  backtest: {
    ...
    execution: {
      strictDataSource: getBoolean('STRICT_DATA_SOURCE', false),
      allowYahooFallback: getBoolean('ALLOW_YAHOO_FALLBACK', true),
      openSlippageBps: getNumber('BACKTEST_OPEN_SLIPPAGE_BPS', 5),
      roundTripFeeBps: getNumber('BACKTEST_ROUND_TRIP_FEE_BPS', 10)
    },
    stability: {
      minSignalAbs: getNumber('MIN_SIGNAL_ABS', 0),
      rebalanceBuffer: getNumber('REBALANCE_BUFFER', 0),
      maxDailyTurnover: getNumber('MAX_DAILY_TURNOVER', 2.0),
      maxPositionAbs: getNumber('MAX_POSITION_ABS', 0.25),
      maxGrossExposure: getNumber('MAX_GROSS_EXPOSURE', 2.0),
      dailyLossStop: getNumber('DAILY_LOSS_STOP', 0)
    },
    /** 連続損失ルール：3 日連続損失でポジション 75% 削減 */
    consecutiveLoss: {
      threshold: getInt('CONSECUTIVE_LOSS_THRESHOLD', CONSECUTIVE_LOSS_RULE.THRESHOLD),
      reduction: getNumber('CONSECUTIVE_LOSS_REDUCTION', CONSECUTIVE_LOSS_RULE.REDUCTION)
    },
    walkforward: {
      trainDays: getInt('WALKFORWARD_TRAIN_DAYS', BACKTEST_DEFAULTS.WALKFORWARD_TRAIN_DAYS),
      testDays: getInt('WALKFORWARD_TEST_DAYS', BACKTEST_DEFAULTS.WALKFORWARD_TEST_DAYS),
      stepDays: getInt('WALKFORWARD_STEP_DAYS', BACKTEST_DEFAULTS.WALKFORWARD_STEP_DAYS)
    },
  ...
  data: {
    ...
    /** yahoo | csv | jquants | stooq（起動時に自動決定。API キーは .env） */
    mode: DEFAULT_DATA_MODE,
    /** 米国 ETF: yahoo | alphavantage | stooq */
  ...
  ops: {
    roleHeader: getString('OPS_ROLE_HEADER', 'x-user-role'),
    notifyWebhook: getString('OPS_NOTIFY_WEBHOOK', ''),
    notifyMinLevel: getString('OPS_NOTIFY_MIN_LEVEL', 'error').toLowerCase(),
    anomalyMaxMissingRatio: getNumber('OPS_ANOMALY_MAX_MISSING_RATIO', 0.2),
    anomalyMaxSpreadPct: getNumber('OPS_ANOMALY_MAX_SPREAD_PCT', 3.0),
    anomalyZScoreThreshold: getNumber('OPS_ANOMALY_ZSCORE_THRESHOLD', 4.0)
  },
  operations: {
    notificationWebhookUrl: getString('OPS_NOTIFY_WEBHOOK', ''),
    /** log | webhook（webhook は OPS_NOTIFY_WEBHOOK 必須） */
    notificationChannel: (() => {
      const v = String(getString('OPS_NOTIFY_CHANNEL', 'log') || 'log').toLowerCase();
      return v === 'webhook' ? 'webhook' : 'log';
    })(),
    notifyOnSignalChange: getBoolean('OPS_NOTIFY_ON_SIGNAL_CHANGE', false),
    activePreset: getString('OPS_ACTIVE_PRESET', 'balanced'),
    profiles: PRESET_PROFILES,
    dataQuality: {
      lookbackDays: getInt('OPS_DATA_QUALITY_LOOKBACK_DAYS', 120)
    },
    operatingRulesPath: getString('OPERATING_RULES_PATH', '')
  },
  preset: {
    active: getString('OPS_ACTIVE_PRESET', 'balanced'),
    profiles: PRESET_PROFILES
  },
...
/**
 * @param {{ mode?: string, dataMode?: string, usOhlcvProvider?: string }} body
 * @returns {{ errors: string[], patch: { mode?: string, usOhlcvProvider?: string } | null }}
 */
function computeDataSourceUpdate(body) {
  const errors = [];
  /** Web は dataMode、内部は mode。どちらか一方だけでも反映する */
  const rawMode =
    body && body.mode !== undefined && body.mode !== null
      ? body.mode
      : body && body.dataMode !== undefined && body.dataMode !== null
        ? body.dataMode
        : undefined;
  const hasMode =
    body &&
  ...
  if (hasMode) {
    ...
    if (!['yahoo', 'csv', 'jquants', 'stooq'].includes(mode)) {
      errors.push('dataMode must be yahoo, csv, jquants, or stooq');
    } else {
      nextMode = mode;
    }
  }
  if (hasUsp) {
    ...
    if (!['yahoo', 'alphavantage', 'stooq'].includes(usp)) {
      errors.push('usOhlcvProvider must be yahoo, alphavantage, or stooq');
    } else {
      nextUsp = usp;
    }
  }
...
applyRecommendedDataSourceSettings();
console.info(
  '[config] データ取得（日本・米国）を自動設定しました（既定は Yahoo 統一・OVERRIDE_* で上書き可）: ' +
    `日本=${config.data.mode} · 米国=${config.data.usOhlcvProvider}`
);
...
function validate() {
  ...
  if (dm !== 'yahoo' && dm !== 'csv' && dm !== 'jquants' && dm !== 'stooq') {
    errors.push('data.mode must be yahoo, csv, jquants, or stooq');
  }

  const usp = config.data.usOhlcvProvider;
  if (usp !== 'yahoo' && usp !== 'alphavantage' && usp !== 'stooq') {
    errors.push('data.usOhlcvProvider must be yahoo, alphavantage, or stooq');
  }
...
/**
 * データ取得の既定は日米とも Yahoo Finance（経路を1本化し、窓不足・複数フォールバックを減らす）。
 * J-Quants / Alpha Vantage / Stooq は OVERRIDE_* のみ（.env にキーがあっても自動では使わない）。
 * @param {typeof config.data} [d]
 * @returns {{ mode: string, usOhlcvProvider: string, reasonJp: string, reasonUs: string }}
 */
function computeRecommendedDataSource(_d = config.data) {
  const oj = String(getString('OVERRIDE_JP_DATA_MODE', '') || '')
    .toLowerCase()
    .trim();
  const ou = String(getString('OVERRIDE_US_OHLCV_PROVIDER', '') || '')
    .toLowerCase()
    .trim();

  let mode;
  let reasonJp;
  if (['yahoo', 'csv', 'jquants', 'stooq'].includes(oj)) {
    mode = oj;
    reasonJp = '環境変数 OVERRIDE_JP_DATA_MODE による固定です。';
  } else {
    mode = 'yahoo';
    reasonJp =
      '既定は Yahoo Finance です。日本・米国で同系統の日足にそろえ、取得経路と運用を単純にします。東証公式データが必要なら OVERRIDE_JP_DATA_MODE=jquants（認証要）を指定してください。';
  }

  let usOhlcvProvider;
  let reasonUs;
  if (['yahoo', 'alphavantage', 'stooq'].includes(ou)) {
    usOhlcvProvider = ou;
    reasonUs = '環境変数 OVERRIDE_US_OHLCV_PROVIDER による固定です。';
  } else {
    usOhlcvProvider = 'yahoo';
    reasonUs =
      '既定は Yahoo Finance です。無料 Alpha Vantage の compact 制限や米国のみ別経路になる分岐を避けます。米国を AV にしたい場合は OVERRIDE_US_OHLCV_PROVIDER=alphavantage です。';
  }

  return { mode, usOhlcvProvider, reasonJp, reasonUs };
}

/**
 * おすすめを config に書き込み runtime JSON に保存
 * @returns {ReturnType<typeof computeRecommendedDataSource>}
 */
function applyRecommendedDataSourceSettings() {
  const rec = computeRecommendedDataSource(config.data);
  config.data.mode = rec.mode;
  config.data.usOhlcvProvider = rec.usOhlcvProvider;
  persistRuntimeDataSourceToFile();
  return rec;
}
...
function getDataSourcesForUi() {
  ...
  const usEffective = (() => {
    ...
    if (d.usOhlcvProvider === 'stooq') {
      return 'Stooq（日足・HTTPS・レートに注意）';
    }
  ...
  const jpEffective = (() => {
    ...
    if (d.mode === 'stooq') {
      return 'Stooq（日足・HTTPS・レートに注意）〔自動〕';
    }
    if (d.mode === 'yahoo') {
      return 'Yahoo Finance（日本・自動選択）';
    }
    if (d.mode === 'jquants') {
      if (hasJquantsCredentials(d)) {
        return `J-Quants（履歴）+ Yahoo（直近 ${d.jquantsRecentWeeksYahoo} 週）〔自動〕`;
      }
  ...
  const rec = computeRecommendedDataSource(d);
  const selectionSummary = `日本=${d.mode} · 米国=${d.usOhlcvProvider}`;
  const recommendation = {
    dataMode: rec.mode,
    usOhlcvProvider: rec.usOhlcvProvider,
    reasonJp: rec.reasonJp,
    reasonUs: rec.reasonUs,
    summary: `日本=${rec.mode} · 米国=${rec.usOhlcvProvider}`,
  ...
  return {
    ...
    recommendation,
    lines: [
      `【自動選択】${selectionSummary}`,
      `選定理由（日本）: ${rec.reasonJp}`,
      `選定理由（米国）: ${rec.reasonUs}`,
...
function get(key, defaultValue = undefined) {
  ...
  for (let i = 0; i < keys.length; i++) {
    if (obj === null || obj === undefined || typeof obj !== 'object' || !(keys[i] in obj)) {
      return defaultValue;
    }
    obj = obj[keys[i]];
  }
