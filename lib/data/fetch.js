...
function withCache(result, ticker, startStr, endStr, mode) {
  if (result.data && result.data.length > 0 && !result.error) {
    dataCache.set(ticker, startStr, endStr, result.data, mode);
  }
  return result;
}
...
async function fetchTickerOhlcv(ticker, days, appConfig, opts = {}) {
  ...
  const cacheMode = appConfig.data.mode === 'csv' ? 'csv' : appConfig.data.mode;
  const cached = dataCache.get(ticker, startStr, endStr, cacheMode);
  if (cached) {
    logger.debug('Cache hit', { ticker, startStr, endStr, mode: cacheMode });
    return { data: cached, error: null, errorCode: null, sourcePath: 'cache' };
  }
  ...
  if (appConfig.data.mode === 'csv') {
    ...
    try {
      ...
      dataCache.set(ticker, startStr, endStr, data, 'csv');
      return { data, error: null, errorCode: null };
    } catch (error) {
  ...
  if (appConfig.data.mode === 'yahoo') {
    if (!ticker.endsWith('.T')) {
      const res = await fetchUsOhlcvWindow(ticker, startStr, endStr, appConfig);
      if (res.data && res.data.length > 0 && !res.error) {
        dataCache.set(ticker, startStr, endStr, res.data, 'yahoo');
      }
      return res;
    }
  ...
  if (appConfig.data.mode === 'jquants') {
    if (!ticker.endsWith('.T')) {
      const res = await fetchUsOhlcvWindow(ticker, startStr, endStr, appConfig);
      if (res.data && res.data.length > 0 && !res.error) {
        dataCache.set(ticker, startStr, endStr, res.data, 'jquants');
      }
      return res;
    }
    ...
    if (startStr > cutoffStr) {
      ...
      return { ...res, sourcePath: 'jp:yahoo_short_window' };
    }
    ...
    if (startStr <= histEndStr) {
      ...
      if (needFallback) {
        ...
        return { ...res, sourcePath: 'jp:jquants_error->yahoo' };
      }
    }
    ...
    dataCache.set(ticker, startStr, endStr, filtered, 'jquants');
  ...
  return { ...res, sourcePath: 'unknown_mode->yahoo' };
}
