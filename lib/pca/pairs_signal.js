...
const { columnMeanStd, zNormalizeVector, EPS } = require('./signal_utils');
...
class PairsSignal {
  ...
  computeSignal(returnsUs, returnsJp, _returnsUsLatest, _sectorLabels, _CFull) {
    try {
      ...
      const { means: jpMean, stds: jpStd } = columnMeanStd(returnsJp);
      ...
      const signalNorm = zNormalizeVector(signal);
