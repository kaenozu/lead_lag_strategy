...
class LeadLagSignal {
  ...
  _validateInputs(returnsUs, returnsJp, returnsUsLatest) {
    ...
    if (nUs < 1 || nJp < 1) {
      throw new Error('Invalid input: US and JP return width must be positive');
    }
    if (returnsUsLatest.length !== nUs) {
      throw new Error(`Dimension mismatch: returnsUsLatest length ${returnsUsLatest.length} !== US width ${nUs}`);
    }
    if (returnsUs.length !== returnsJp.length) {
      throw new Error('Invalid input: returnsUs and returnsJp must have the same number of rows');
    }
