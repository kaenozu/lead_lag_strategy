...
class LeadLagSignal {
  ...
  _validateInputs(returnsUs, returnsJp, returnsUsLatest) {
    ...
    // Validate dimensions
    const expectedUsSectors = 11;
    const expectedJpSectors = 17;
    
    if (!Array.isArray(returnsUs[0])) {
      throw new Error('Invalid input: returnsUs must be 2D array');
    }
    
    if (returnsUs[0].length !== expectedUsSectors) {
      throw new Error(`Dimension mismatch: expected ${expectedUsSectors} US sectors, got ${returnsUs[0].length}`);
    }
    
    if (!Array.isArray(returnsJp[0])) {
      throw new Error('Invalid input: returnsJp must be 2D array');
    }
    
    if (returnsJp[0].length !== expectedJpSectors) {
      throw new Error(`Dimension mismatch: expected ${expectedJpSectors} JP sectors, got ${returnsJp[0].length}`);
    }
    
    if (returnsUsLatest.length !== expectedUsSectors) {
      throw new Error(`Dimension mismatch: expected ${expectedUsSectors} US latest returns, got ${returnsUsLatest.length}`);
    }
    
    // Validate sufficient history
    const minHistory = this.config.windowLength || 60;
    if (returnsUs.length < minHistory) {
      throw new Error(`Insufficient history: ${returnsUs.length} days < required ${minHistory} days`);
    }
    if (returnsJp.length < minHistory) {
      throw new Error(`Insufficient history: ${returnsJp.length} days < required ${minHistory} days`);
    }
    
    // Check for NaN/Infinity values
    for (let i = 0; i < Math.min(5, returnsUs.length); i++) {
      for (let j = 0; j < returnsUs[i].length; j++) {
        if (!Number.isFinite(returnsUs[i][j])) {
          throw new Error(`Invalid value in returnsUs[${i}][${j}]: ${returnsUs[i][j]}`);
        }
      }
    }
  }
