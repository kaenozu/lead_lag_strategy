...
module.exports = {
  ...
  rollingVolScale,
  applySignalVolNorm,
  // consumer-compatible aliases
  columnMoments: computeColumnMoments,
  zScoreRow: (row, mean, std) => row.map((v, j) => (v - mean[j]) / (std[j] || EPS)),
  meanAndStd: (arr) => {
    const n = arr.length;
    if (!n) return { mean: 0, std: 0 };
    const m = arr.reduce((a, b) => a + b, 0) / n;
    const s = Math.sqrt(arr.reduce((sq, x) => sq + (x - m) ** 2, 0) / n + EPS);
    return { mean: m, std: s };
  },
  normalizeByMaxAbs: normalizeMaxAbs,
  columnMeanStd: computeColumnMoments,
  computeColumnStats: computeColumnMoments,
  safeNormalizeSignal: normalizeStd,
  matrixMeanStd: computeColumnMoments,
  computeSignalMoments: (arr) => {
    const n = arr.length;
    if (!n) return { mean: 0, std: 0 };
    const m = arr.reduce((a, b) => a + b, 0) / n;
    const s = Math.sqrt(arr.reduce((sq, x) => sq + (x - m) ** 2, 0) / n + EPS);
  ...
  marketAverage: (row) => row.reduce((a, b) => a + b, 0) / row.length
};
