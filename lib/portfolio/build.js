...
const logger = createLogger('PortfolioBuild');

/**
 * ロングショートポートフォリオの構築
 * Portfolio Building with Long-Short Positions
 *
 * @param {Array<number>} signal - シグナル配列
 * @param {number} quantile - 分位点（0.0 - 0.5）
 * @returns {Array<number>} ポートフォリオウェイト
 */
function buildPortfolio(signal, quantile = 0.4) {

  if (!signal || signal.length === 0) {
    logger.warn('Invalid signal: signal is empty or null');
    return new Array(0);
  }

  if (quantile <= 0 || quantile >= 0.5) {
    logger.warn(`Invalid quantile: ${quantile}, must be > 0 and < 0.5, using default 0.4`);
    quantile = 0.4;
  }

  const n = signal.length;
  // 論文の「上位 q%」を正確に再現：四捨五入を使用
  // 例：n=17, quantile=0.4 の場合、floor(6.8)=6 ではなく round(6.8)=7 銘柄
  const q = Math.max(1, Math.round(n * quantile));

  // エッジケース：全シグナル値が同一の場合、ニュートラルなウェイトを返す
  const allSame = signal.every((v) => v === signal[0]);
  if (allSame) {
    logger.warn('All signals are identical, returning neutral weights');
    return new Array(n).fill(0);
  }

  // エッジケース：すべてのシグナルがゼロまたは極小値
  const maxSignal = Math.max(...signal.map(Math.abs));
  if (maxSignal < 1e-10) {
    logger.warn('All signals are near zero, returning neutral weights');
    return new Array(n).fill(0);
  }

  const indexed = signal.map((val, idx) => ({ val, idx }));
  indexed.sort((a, b) => {
    // 数値的安定性：イプシロン未満の差は同一とみなす
  ...
  if (longIdx.length !== shortIdx.length) {
    logger.debug(`Imbalanced portfolio: ${longIdx.length} long, ${shortIdx.length} short`);
  }
...
function buildDoubleSortPortfolio(momentumSignal, pcaSignal, quantile = 0.4) {
  ...
  const allSame = momentumSignal.every((v) => v === momentumSignal[0]) &&
                  pcaSignal.every((v) => v === pcaSignal[0]);
  if (allSame) {
