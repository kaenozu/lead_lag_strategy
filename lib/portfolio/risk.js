 ...
 *
 * 計算ロジック:
 * - ターンオーバー = Σ|w_new - w_old| / 2
 *   例：ロング 10%→20%、ショート 10%→0% なら、ターンオーバー = (|0.2-0.1| + |-0.1-0|) / 2 = 0.1 = 10%
 * - コスト = (slippage + commission) × ターンオーバー
 *   注：ターンオーバー計算時に既に両方向取引を考慮しているため、追加の×2 は不要
 */
function applyTransactionCosts(ret, costs, prevWeights, currWeights) {
  ...
  // 初期構築時（prevWeights なし）: 全ウェイトが 0 から構築されるため、ターンオーバー = グロスエクスポージャー
  if (!prevWeights) {
    // currWeights もない場合はコスト 0（取引なし）
    if (!currWeights) {
      return ret;
    }
    const grossExposure = currWeights.reduce((sum, w) => sum + Math.abs(w), 0);
    logger.debug('Applying initial transaction cost', {
      slippage,
      commission,
      grossExposure,
      totalCost: totalCostRate * grossExposure
    });
    return ret - totalCostRate * grossExposure;
  }

  // currWeights がない場合もコスト 0（取引なし）
  if (!currWeights) {
    return ret;
  }
